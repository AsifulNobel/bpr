<?php
	...
	while ($row = mysqli_fetch_assoc($result)) {
		...
		$list .= '<tr>
					<td>'.$serial.'</td>
					<td class="btn btn-default custom-button" onclick="user_list_by_role('.$row['role_id'].')"><input type="hidden" id="role_id-'.$row['role_id'].'" value="'.$row['role_name'].'">'.$row['role_name'].'</td>';
